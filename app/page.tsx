...
import { MapPin, Search, Loader2, AlertTriangle, RefreshCw, Megaphone } from 'lucide-react';
...
const AREAS = ['Victoria Island', 'Lekki', 'Ikoyi', 'Yaba', 'Ikeja', 'Surulere', 'Ajah', 'Eko Atlantic'] as const;
    ...
    return (
      <div className="flex flex-col items-center gap-4 px-6 py-[48px] text-center">
        ...
        <div className="max-w-[280px] text-sm" style={{ color: '#A7A8B5' }}>
          Something went wrong fetching the latest parties. Try again in a moment.
        </div>
  ...
  if (empty) {
    return (
      <div className="flex flex-col items-center gap-4 px-6 py-[48px] text-center">
        ...
        <div className="max-w-[280px] text-sm" style={{ color: '#A7A8B5' }}>
          No events are live right now. Check back soon — or list your own event to get in front of the Lagos Live crowd.
        </div>
        <Link href="/host" className="btn-primary px-6 py-3 text-sm font-semibold">
          List Your Event
        </Link>
      </div>
    );
...
export default function HomePage() {
  ...
  return (
    <div className="animate-fade-in">
      ...
      {/* Hero */}
      <div className="relative overflow-hidden px-[22px] pb-[42px] pt-[60px]" onMouseMove={onHeroMove} onMouseLeave={onHeroLeave}>
        <div
        ...
        />
        <div className="pointer-events-none absolute -right-[30px] -top-10 h-[250px] w-[250px]">
          <div
            className="h-full w-full rounded-full transition-transform duration-200 ease-out"
            style={{
              background: 'radial-gradient(circle, rgba(255,45,149,0.15), transparent)',
              filter: 'blur(20px)',
              transform: `translate(${hero.x * 25}px, ${hero.y * 25}px)`,
            }}
          />
        </div>
        <div className="pointer-events-none absolute left-[5px] -bottom-[25px] h-[180px] w-[180px]">
          <div
            className="h-full w-full rounded-full transition-transform duration-200 ease-out"
            style={{
              background: 'radial-gradient(circle, rgba(138,43,226,0.12), transparent)',
              filter: 'blur(20px)',
              transform: `translate(${hero.x * -18}px, ${hero.y * -18}px)`,
            }}
          />
        </div>
        <div className="relative z-[1] max-w-[540px]">
          <div
            className="mb-[18px] inline-flex items-center gap-[7px] rounded-full px-4 py-1.5"
          ...
          <h1
            ...
            style={{ fontSize: 'clamp(64px,14vw,110px)', fontWeight: 400 }}
          >
            <span style={{ color: '#FFFFFF' }}>{heroCopy.line1} </span>
            <br />
            <span className="gradient-text">
              {heroCopy.line2}
            </span>
          </h1>
          <p className="mb-7 max-w-[400px] text-[15px] leading-[1.7]" style={{ color: '#A7A8B5' }}>
            Discover the hottest parties, clubs &amp; events across Lagos — right now.
          </p>
          <button
            onClick={requestLocation}
            className="btn-primary flex items-center gap-2 px-8 py-[15px] text-[15px]"
          >
            {locationLoading ? (
      ...
      <div className="no-scrollbar flex gap-2 overflow-x-auto px-5 pb-4">
        {QUICK_FILTERS.map(({ label, href }, i) => (
          <Link
            key={label}
            href={href}
            className="whitespace-nowrap rounded-full px-4 py-2 text-[13px] font-medium outline-none transition-all duration-300 active:scale-95"
            style={{
              background: i === 0 ? 'rgba(255,45,149,0.12)' : 'rgba(255,255,255,0.05)',
              border: '1px solid',
              borderColor: i === 0 ? 'rgba(255,45,149,0.3)' : 'rgba(255,255,255,0.08)',
              color: i === 0 ? '#FF2D95' : '#A7A8B5',
            }}
          >
            {label}
          </Link>
        ...
        <div className="no-scrollbar flex gap-2 overflow-x-auto pb-1">
          {AREAS.map((area) => (
            <Link
              ...
              className="whitespace-nowrap rounded-full px-3.5 py-[7px] text-[12px] font-medium transition-all duration-200 active:scale-95"
      ...
      <Link href="/search" className="block px-5 pb-5">
        <div
          className="flex items-center gap-2.5 rounded-xl px-4 py-[11px] transition-all duration-200 hover:border-[rgba(255,45,149,0.2)]"
          style={{
        ...
        >
          ...
          <span className="text-sm" style={{ color: '#6B6C80' }}>
      ...
      {/* List your event CTA */}
      <Link href="/host/new" className="block px-5 pb-5">
        <div
      ...
      {/* Trending ticker */}
      <div className="mb-4 border-y py-2.5" style={{ borderColor: 'rgba(255,255,255,0.04)' }}>

...
type ScannerState = 'starting' | 'scanning' | 'denied' | 'stopped';
    ...
    const boot = async () => {
      ...
      try {
        ...
        const onScan = (text: string) => {
          ...
          if (busyRef.current || (now - (feedbackShownAt.current ?? 0)) < 900) return;
        ...
        scannerRef.current = {
          start: async () => {
            ...
            if (!disposed) setScanner('scanning');
          },
          stop: async () => {
            try {
              await instance.stop();
          ...
          {feed ? (
            ...
            <div className="flex h-[300px] flex-col items-center justify-center gap-3 px-6 text-center">
              ...
              <p className="max-w-[300px] text-[13px] leading-relaxed" style={{ color: ci.muted }}>
                Allow camera access when your browser asks, or enable it in site settings. In Safari/Chrome: tap the icon in the address bar → Camera → Allow.
              </p>
              <button
                onClick={() => {
                  setScanner('starting');
            ...
            <div className="relative mx-4 mb-4 overflow-hidden rounded-2xl" style={{ background: '#000' }}>
              ...
              <div className="pointer-events-none absolute inset-0 z-[1] border-[3px] rounded-2xl ci-corners" />

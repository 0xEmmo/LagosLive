...
// event so this component paints immediately while the viewer's real distance
...
export default function PartyDetailClient({
  ...
  const { party, loading, error, retry } = useParty(partyId, initialParty);
  ...
  const shareUrl = partyShareUrl(party);
        ...
        <div className="mb-6 glass rounded-2xl p-4" style={{ background: 'rgba(255,255,255,0.03)' }}>
          ...
          <div className="flex flex-wrap gap-2.5">
            ...
            <button
              onClick={copyLink}
              className="flex items-center gap-1.5 rounded-[10px] px-3.5 py-2.5 text-[13px] font-medium glass glass-hover"
              style={{ color: copied ? '#00F5D4' : '#A7A8B5' }}
            >
